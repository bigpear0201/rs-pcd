...
fn main() -> Result<()> {
    ...
    if let Some(ts_col) = block.get_column("timestamp") {
        if let Some(timestamps) = ts_col.as_f64() {
            println!("前 5 个点的时间戳:");
            for (i, timestamp) in timestamps.iter().enumerate().take(block.len.min(5)) {
                println!("  点 {}: {:.6}", i, timestamp);
            }
        }
    }
    ...
    if let Some(ring_col) = block.get_column("ring") {
        if let Some(rings) = ring_col.as_u16() {
            println!("\n前 5 个点的线束编号:");
            for (i, ring) in rings.iter().enumerate().take(block.len.min(5)) {
                println!("  点 {}: ring {}", i, ring);
            }
        }
    }
    ...
    #[cfg(feature = "rayon")]
    {
        ...
        if let Some((x, y, z)) = block.xyz() {
            ...
            for (i, distance) in distances.iter().enumerate().take(5.min(distances.len())) {
                println!("  点 {}: {:.3} 米", i, distance);
            }
        }
    }
